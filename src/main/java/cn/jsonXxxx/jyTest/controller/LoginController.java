...
import cn.jsonXxxx.jyTest.shiro.MyUsernamePasswordToken;
...
public class LoginController {
    ...
    @PostMapping("/login")
    @ResponseBody
    public Result LoginTest(String username, String password, HttpServletRequest request, HttpServletResponse response) {
        // 返回的状态
        ...
        if (!currentUser.isAuthenticated()) {
            ...
            if (StringUtils.isNotBlank(username)) {
                token = new MyUsernamePasswordToken(username, password);
                // remembermMe记住密码
                token.setRememberMe(true);
            } else {
                token = new MyUsernamePasswordToken("ad");
            }
            try {
                // 执行登录.
